  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <button className="text-sm font-medium text-foreground bg-card border border-card-border hover:bg-sidebar-accent px-4 py-2 rounded-lg transition-colors flex items-center gap-2">
        ...
        <div className="divide-y divide-card-border">
          {notifications.map((notification) => (
            <div 
              key={notification.id} 
              className={cn(
                "p-5 flex gap-4 transition-colors hover:bg-sidebar-accent/30 relative",
            ...
            >
              ...
              <div className={cn("w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 mt-1", getIconBg(notification.type))}>
                {getIcon(notification.type)}
              </div>
              
              <div className="flex-1 min-w-0">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-1">
                  <h4 className={cn("text-base font-semibold", !notification.read ? "text-foreground" : "text-foreground/80")}>
                    {notification.title}
                  </h4>
                  <span className="text-xs font-medium text-sidebar-fg">{notification.time}</span>
                </div>
                <p className={cn("text-sm", !notification.read ? "text-sidebar-fg" : "text-sidebar-fg/70")}>
                ...
                {!notification.read && (
                  <div className="mt-3 flex items-center gap-3">
                    ...
                    <button className="text-xs font-medium text-sidebar-fg hover:text-foreground transition-colors">
                      Mark as read
                    </button>
                  </div>
                )}
              ...
              <button className="text-sidebar-fg hover:text-foreground transition-colors p-1.5 h-fit rounded-lg hover:bg-sidebar-accent opacity-0 group-hover:opacity-100 focus:opacity-100">
